...
<body>
<form class="n-weblist" method="post" action="<?=$this->url('action=place_order')?>" id="webaddr">
	<div class="n-personal-center-tit">
		<a href="javascript:history.go(-1);"><img width="11" height="21" src="<?php echo static_file('mobile/img/img-22.png'); ?> " alt=""></a>
		收货地址管理
		<input class="n-keep" value="管理" type="button" id="manage_addr">
	</div>
	<div class="n-h5"></div>
	...
	<div class="n-shopping-box">
		<div class="n-shopping-box-top">
			<input type="checkbox" name="addr_id" value="<?=$row['id']?>" data-phone="<?php echo $row['phone']?>" data-addr="<?php echo $row['address']?>" />
		</div>
...
$(function(){
	...
	$('#addr_check').click(function() {
		/*var add_id = $('input[name=addr_id]:checked').val();
		var phone = $('input[name=addr_id]:checked').attr('data-phone');
		var addr = $('input[name=addr_id]:checked').attr('data-addr');
		if(!add_id || !phone || !addr) {
			alert("请选择正确的收货地址");
			return false;
		}*/
		$('#webaddr').submit();
	});
	$('#manage_addr').click(function() {
		$.cookie('buy_shop','buy_shop',{path: "/",expires: 7});
		window.location = '<?=$this->url('usercp/address')?>';
		return false;
	});
})
</script>
